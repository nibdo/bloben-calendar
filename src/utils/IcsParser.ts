...
const IcsParser: any = {
    ...
    enhanceParsedEvent: (eventArray: any, calendar: CalendarStateEntity) => {
      const result: any = [];
    ...
    extractEvents: (icsFile: string): string => {
        ...
        const firstEventIndex: number = icsFile.indexOf(
            EVENT_BEGIN_KEY_VALUE)
        const endOfEventsIndex: number = icsFile.indexOf(
            CALENDAR_END_KEY_VALUE);

        return icsFile.slice(firstEventIndex,
                             endOfEventsIndex);
    },
    /**
    ...
    transformValue: (key: string, value: string): string => {
        if (key === 'startAt' || key === 'endAt' || key === 'createdAt' || key === 'updatedAt') {
    ...
    splitToKeyValueObj: (item: string) => {
        ...
        if (isDateKey) {
            delimiterIndex = item.indexOf('TZID') !== -1
                ? item.indexOf(
                ';')
                : item.indexOf(':');
        } else {
